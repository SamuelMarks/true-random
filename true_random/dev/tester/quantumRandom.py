...
class QuantumRandom(Random):
    ...
    def _fetch(self, dataType, arrayLength=1, blockSize=1):
        ...
        if dataType not in self._DATATYPES:
            raise ValueError('dataType must be one of %s' % self._DATATYPES)
        if arrayLength > self._MAXLEN:
            raise ValueError('arrayLength cannot be larger than %d' % self._MAXLEN)
        if blockSize > self._MAXLEN:
            raise ValueError('blockSize cannot be larger than %d' % self._MAXLEN)
